...
{
    public partial class PCMSelfServiceForm : BasePage
    {
        ...
        {
            workNo = CurrentUserInfo.Personcode;
            hidWorkNo.Value = workNo;
            DataTable dt = kqmSelfServiceBll.getEmpInfo(workNo, this.SqlDep, Privileged);
            string alert = "";
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                string PlaceName = "";
                DataTable dataTablePlaceName = kqmSelfServiceBll.getPlaceName(workNo);
                ...
                string EffectBellNo = "";
                DataTable dataTableEffectBellNo = kqmSelfServiceBll.getEffectBellNo(workNo);
                ...
                {
                    DataTable dataTableEffectBellNo2 = kqmSelfServiceBll.getEffectBellNo2(workNo);
                ...
                DataTable dataTableKaoQinData = kqmSelfServiceBll.getKaoQinData(workNo);
                UltraWebGridKaoQinQuery.DataSource = dataTableKaoQinData;
                UltraWebGridKaoQinQuery.DataBind();
                DataTable dataTableScoreItemData = kqmSelfServiceBll.getScoreItemData(workNo);
                WebGridScoreItem.DataSource = dataTableScoreItemData;
                WebGridScoreItem.DataBind();
                DataTable dataTableLeaveReportData = kqmSelfServiceBll.getLeaveReportData(workNo);
                UltraWebGridLeaveReport.DataSource = dataTableLeaveReportData;
                UltraWebGridLeaveReport.DataBind();
                DataTable dataTableWorkShiftData = kqmSelfServiceBll.getWorkShiftData(workNo);
                UltraWebGridWorkShift.DataSource = dataTableWorkShiftData;
                UltraWebGridWorkShift.DataBind();
                DataTable dataTableLeaveDetailData = kqmSelfServiceBll.getLeaveDetailData(workNo);
                UltraWebGridLeaveDetail.DataSource = dataTableLeaveDetailData;
                UltraWebGridLeaveDetail.DataBind();
                DataTable dataTableOTMMonthTotalData = kqmSelfServiceBll.getOTMMonthTotalData(workNo);
                UltraWebGridOTMMonthTotal.DataSource = dataTableOTMMonthTotalData;
                UltraWebGridOTMMonthTotal.DataBind();
                DataTable dataTableOTMonthDetailData = kqmSelfServiceBll.getOTMonthDetailData(workNo);
        ...
        protected void WebGridScoreItem_DataBind(object sender, EventArgs e)
        {
            ...
            {
                sURL = "../../KQM/Query/PAMEmpQryForm.aspx?WorkNo=" + this.WebGridScoreItem.Rows[i].Cells.FromKey("WorkNo").Text + "&Year=" + this.WebGridScoreItem.Rows[i].Cells.FromKey("ANNUALCode").Text;
