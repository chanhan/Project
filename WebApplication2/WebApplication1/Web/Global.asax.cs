...
namespace Maticsoft.Web 
{
	...
	public class Global : System.Web.HttpApplication
	{
        ...
        private Maticsoft.BLL.CustomerData CustomerDataBll = new Maticsoft.BLL.CustomerData();
        private Maticsoft.BLL.CodeData CodeDatadal = new Maticsoft.BLL.CodeData();
		...
		{		
           #region 默认蓝
            ...
            #region 初始化客户信息到哈希表
            DataTable dtCustomer = CustomerDataBll.GetList("").Tables[0];
            foreach (DataRow dr in dtCustomer.Rows)
            {
                HtCustomer.Add(dr["PhoneNumber"], recivemsgbll.DataRowToCustomerModel(dr));
            }
            Application["App_Customer"] = HtCustomer;
			#endregion 	
	
            #region 初始化代码信息到哈希表
            DataTable dtCode = CodeDatadal.GetList("").Tables[0];
            foreach (DataRow dr in dtCode.Rows)
            {
                HtCode.Add(dr["code"], recivemsgbll.DataRowToCodeModel(dr));
            }
            Application["App_Code"] = HtCode;
            #endregion 
